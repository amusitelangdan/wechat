        ...
        <div style="box-shadow: 0 0 0.1rem #a6a6a6;background: #e6e6e6;padding: .6rem 0;" @touchmove.prevent>
          ...
          <el-row type="flex" align="middle">
            <el-col class="numberStyle" @click.native="inputPsd('删除')">
              <div class="number">
                <img src="../../../../assets/img/icon/global/delete_button.png" alt="" style="width: 2rem;height: 1.5rem;margin: 1.3rem 0 0 3rem;display: block">
              </div>
            </el-col>
...
<script>
  export default {
      ...
      inputPsd(value) {
        if (value === '完成') {
          if (this.temperatureGroup.length <= 0) {
            ...
            this.$emit('input', `${this.temperatureGroup[0]}${this.temperatureGroup[1]}.${this.temperatureGroup[2]}℃`);
          }
    ...
    created() {
      ...
      if (this.valueInside !== '' && this.valueInside !== undefined) {
        this.temperatureGroup[0] = this.valueInside.substring(0, 1);
        this.temperatureGroup[1] = this.valueInside.substring(1, 2);
        this.temperatureGroup[2] = this.valueInside.substring(3, 4);
      }
    },
  };
</script>
